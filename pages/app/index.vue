...
const currentDay = computed(() => {
  if (!store.value)
    return
  const day = store.value.days.find(day => dayjs().subtract(testDay, 'day').isSame(day.time, 'day'))
  if (day)
    return day
})
const currentWeek = computed(() => {
  if (!store.value)
    return
  const week = store.value.weeks.find(week => theSameWeek(dayjs().subtract(testDay, 'day').valueOf(), week.time))
  if (week)
    return week
})
...
.goal-button:hover, .goal-button.active {
